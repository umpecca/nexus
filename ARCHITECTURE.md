...
- `electron/mcp-server.cjs`: Embedded Model Context Protocol server. Owns the `http.Server` bound to `127.0.0.1` on the configured port, implements the Streamable HTTP transport at `POST /mcp` with JSON-RPC framing, validates the bearer token, dispatches to the read-only tools (`nexus_list_windows`, `nexus_get_document`, `nexus_get_outline`, `nexus_get_section`, `nexus_search_document`, `nexus_get_selection`) and the write tool (`nexus_replace_document`), and exposes start/stop/reconfigure hooks used by `electron/main.cjs`. The tool implementations delegate to host callbacks supplied by the main process; the write tool and the selection read tool ask the target renderer (to confirm a replacement, or to report the live selection) before resolving.
...
- Expand the MCP write tool surface (patch/find-replace edits, save/save-as tools, image-aware writes) now that the read tools cover outline, section, search, and selection alongside list-windows and get-document.
