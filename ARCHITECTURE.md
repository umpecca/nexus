...
- `src/lib/openapiYaml.ts`: Shared OpenAPI fence matching, YAML parsing/required-field validation,
  serialization, default-document creation, rich-text summary extraction, and construction of the
  read-only API-reference model, including local `$ref` resolution and path/operation parameter
  merging.
- `src/lib/openapiRequestBody.ts`: Direct mutation and validation helpers for the visual route
  request-body form. They retain complete media/property nodes during rename operations, manage
  inline-schema required lists, map familiar payload formats to guided media-type choices, and parse
  JSON examples before the visual editor updates its draft.
- `src/components/editor/OpenApiReferencePreview.tsx`: Dependency-free React rendering for the
  expandable, Swagger-style document preview. It uses ordinary local HTML/CSS, does not load remote
  assets, and deliberately omits request execution.
- `electron/openapiExport.cjs`: Main-process export renderer for explicitly marked YAML OpenAPI
  fences. It validates the essential OpenAPI 3 structure and emits the same route, parameter,
  request-body, response, security, and locally resolved schema information as print-safe static
  HTML; ordinary YAML and invalid specifications continue through Marked's code renderer.
...
- `electron/aiChatHistory.cjs`: Pure CommonJS helper for opaque per-profile/per-document AI chat-history paths and safe transcript/agent-message sanitization. Main-process IPC reads and writes the resulting JSON files under userData's `ai-chats` directory; API keys are never included. Covered by `electron/aiChatHistory.test.ts`.
...
- `src/components/editor/PrintPreviewDialog.tsx`: Large in-app PDF snapshot viewer with loading and
  inline error states, Refresh and Save PDF actions, and object-URL cleanup when snapshots are
  replaced or the dialog closes.
...
- `src/components/settings/AiSettingsDialog.tsx`: AI provider setup, including OpenCode server/username/password/agent/provider/model fields, live discovery with manual identifier fallbacks, connection testing, saved-chat-history deletion, and a warning that OpenCode tools run in its served directory. OpenCode sampling controls are hidden because its agent/model configuration owns them.
- `src/components/ai/AiChatPanel.tsx`: Document-scoped chat UI and renderer conversation owner. Direct providers retain the Nexus MCP agent loop; OpenCode receives a stable conversation ID and read-only provider-tool activity because OpenCode executes its own tool loop. Saved-document transcript and agent state is restored from user-data, while untitled documents stay session-only.
...
- `src/styles.css`: Global application styling, including light and dark app theme tokens, the toggleable continuous paper/plain rich-text editing surface, the PDF preview dialog, toolbar-matched rich/source/diff editor backgrounds, edge-to-edge editor frame, sticky white shadcn-styled grouped toolbar layout with a gray bottom border, floating source/diff view controls, bordered paragraph dropdown controls, raised transform-offset dropdown and tooltip layers, the collapsible outline sidebar layout, and the shared Publish as Web / QuickConnect dialog layout.
...
15. For File/Export as HTML, File/Export as PDF, and File/Print Preview, the focused renderer sends its current Markdown buffer, current file path, selected editor font, selected base font size, and selected paragraph spacing to Electron without changing saved state. PDF export and preview also send the selected Letter or A4 paper size, portrait/landscape orientation, and page margins.
16. The Electron main process renders export HTML with Marked, resolves Markdown image paths with the same local path rules used by preview, converts supported admonition directives into styled callout HTML, and emits Mermaid placeholders for fenced `mermaid` code blocks. HTML export uses a self-contained path: prompt for the destination, inline supported local Markdown images as base64 data URLs, inline selected bundled Fontsource CSS and font files as base64 data URLs, load the document from a temporary file for Mermaid rendering, replace successful Mermaid placeholders with base64 SVG image data URLs, and write the static HTML file. PDF export and Print Preview share one direct renderer: strip leading YAML frontmatter, load the rendered HTML as a data URL, render Mermaid diagrams in that export window, wait for fonts, apply print fragmentation hints that keep headings with following content, preserve paragraph/list widows and orphans, and avoid splitting common blocks when possible, then call Electron's PDF renderer once with selected page size, orientation, custom margins, and print backgrounds. Export writes those bytes after a save dialog; preview returns them to an in-app PDF viewer and can save that exact snapshot later. If direct rich PDF generation fails, the operation reports that error instead of creating a text-only fallback.
...
34. Mermaid code blocks are represented as standard fenced `mermaid` blocks, rendered as non-editable SVG diagrams in rich text mode, and edited as raw Markdown in source or diff mode. The shared drawio/isoflow toolbar workflow captures both the active root-or-nested Lexical editor and a cloned range selection before opening the asynchronous native diagram window, validates and restores that selection when the SVG returns, and falls back to the target editor's end only when no usable caret remains.
34a. Embedded OpenAPI specifications use a `yaml openapi` fenced block. A custom code-block
descriptor renders a title/version/route/schema header plus an expandable, read-only API reference
and reopens a modal Electron host; new-block insertion reuses the asynchronous native-editor
selection capture so root and nested carets survive the modal round trip. The reference resolves
local component references and groups method-colored operations by tag while remaining offline. The
host mutates a cloned generic YAML object rather than projecting through a lossy schema, preserving
unsupported OpenAPI fields and `x-*` extensions through visual edits.
The route editor exposes OpenAPI 3 request-body controls for media types, component references or
inline object schemas, required fields, and JSON examples; a component request-body `$ref` is left
intact until the user explicitly replaces it with an operation-local body.
The payload-format selector maps common choices to their MIME types and presents contextual guidance,
while retaining unknown vendor media types as explicitly editable Custom values.
34b. HTML, PDF, and web-publish rendering replaces explicitly marked valid OpenAPI fences with an
expanded static API reference. It deliberately omits edit, collapse, and request-execution controls,
and uses local method-color and page-break-safe export CSS; malformed OpenAPI and ordinary YAML
fences continue through the normal fenced-code renderer.
...
Description: Adds File menu actions for rendered HTML/PDF exports and Print Preview of the current Markdown buffer. The renderer sends the active Markdown, current file path, selected editor font, selected base font size, and selected paragraph spacing through preload. PDF export and preview also send the selected Letter or A4 paper size, portrait/landscape orientation, and page margins. The Electron main process renders a styled HTML document with Marked, resolves Markdown image paths relative to the opened Markdown file when possible, converts supported admonition directives into callout HTML, and turns fenced Mermaid blocks into export placeholders. HTML export prompts for a destination first, embeds supported local images and bundled web font files as base64 data URLs, renders Mermaid placeholders in a hidden BrowserWindow using Mermaid's browser bundle, replaces successful diagrams with base64 SVG image data URL `<img>` elements, and writes a self-contained static HTML file. PDF export and preview share one direct path: strip leading YAML frontmatter metadata, create one hidden export window, load the rendered HTML as a data URL, render Mermaid placeholders as inline SVG in that window, wait for fonts, and call `printToPDF` once with the selected page size, orientation, custom margins, and print backgrounds. Export writes the result after a destination prompt; preview displays the same bytes in-app and can save the displayed snapshot. If direct rich printing fails, the operation reports the failure and does not create a text-only fallback PDF.
...
#### PDF and Image Document Import Workflow
