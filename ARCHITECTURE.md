...
- `src/components/editor/OutlineSidebar.tsx`: Shadcn-styled collapsible outline panel that lists the current document's headings as a depth-indented, clickable tree and reports the selected heading so the app shell can scroll the rich-text editor to it.
...
- `src/components/editor/ShadcnMdxToolbar.tsx`: Project-owned shadcn-styled grouped toolbar composition that keeps MDXEditor's broad rich-text command set visible in unlabeled button groups, applies consistent tooltips to Nexus-owned controls, includes paper/plain, paper orientation, plain-view responsive wrapping, and outline sidebar toggles, floats the view-control group in source and diff modes, and leaves undo, redo, refresh, zoom, and document actions in native menus.
...
- `src/lib/outline.ts`: Extracts the document outline (heading level, text, and ordinal position) from the current Markdown buffer for the outline sidebar, ignoring headings inside fenced code blocks.
...
53. When the outline sidebar is enabled and the editor is in rich-text mode, the app shell extracts the current document's headings (level, text, and ordinal position) from the Markdown buffer with a helper that skips fenced code blocks, recomputes them as the document changes, and passes them to the outline sidebar. The sidebar shows an empty-state message when no headings exist and is not rendered in source or diff modes.
54. Selecting an outline entry asks the app shell to scroll the matching heading into view; the shell finds the rendered heading element at the selected ordinal position inside the active rich-text editor scroll container and scrolls to it using the same editor scroll-into-view helpers used by find navigation. The outline sidebar's open/closed state is read on launch and persisted per OS profile through the settings store, defaulting to hidden when no stored value is available.
...
Deployment: Runs across each renderer window (the QuickConnect dialog) and the Electron main process (HTML render and HTTP POST). The bearer token is stored in per-profile local settings; the URL, path, and token are sent to the main process per publish.
...
Description: Lets the user toggle a collapsible sidebar beside the rich-text editor that lists the current document's headings as a depth-indented, clickable tree. The app shell extracts headings (level, text, and ordinal position) from the current Markdown buffer with a small helper that ignores headings inside fenced code blocks, and recomputes them as the document changes. Selecting an entry scrolls the rich-text editor to the corresponding heading by locating the rendered heading element at the matching ordinal position in the active editor scroll container and reusing the editor scroll-into-view helpers shared with find navigation. The sidebar shows an empty-state message when the document has no headings, is rendered only in rich-text mode (hidden in source and diff modes so those modes keep the full editor area), and persists its open/closed state per OS profile through the settings store.

Technologies: React state, MDXEditor change events, Markdown heading parsing helper, DOM scroll helpers, local storage.
...
- QuickConnect persists its bearer token in local settings in plaintext by explicit user choice for convenience; this is a deliberate exception to the never-store-secrets posture and is called out so users understand the token is readable in local storage. Prefer HTTPS endpoints so the token and document are not sent in the clear.
...
- Offer optional OS-keychain / encrypted storage for the QuickConnect bearer token instead of plaintext local settings.
