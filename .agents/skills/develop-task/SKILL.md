...
Your task is to generate the code implementation for a given task based on the latest project state, documentation, and the task document provided. Keep the answers concise, concrete, and unambiguous where possible.

Retrive the task document by using the task ID from the prompt and the tasks directory in the root of the project.

- If a task ID or prompt is not provided, go to next unclosed task in line.
- If a task ID is not given, but a plain text prompt is given, create a task document based on the prompt and ask for confirmation before proceeding with coding. Number this new task document with a new unique task ID and save it in the tasks directory. Proceed to complete the task based on this new task document.
- If a task document is not found, ask for clarification or a different task ID.
- If there is no next unclosed task in line: ask for a task ID or prompt to create a new task document or to cancel.
